...
> 履历对应阶段：2024.09–2024.12（2024.09 起与 Dify 项目双轨验证，代码与 Git 提交在当前日期重新整理，不伪造历史提交时间）
...
| 标签 | 履历对应阶段 | 业务演进 | 技术成熟度口径 |
| --- | --- | --- | --- |
| `v0.1.0-rag-service-desk` | 2024.09–2024.12 | 从知识问答扩展到状态查询、人工审批建单和审计闭环 | 自主 RAG 底座；LangGraph 属于项目内工程验证 |
| `v0.2.0-mcp-pilot` | 2025.01–2025.06 | 将既有只读业务能力标准化，减少调用方重复适配 | MCP 内部试点，不描述为国内大规模应用 |
| `v0.3.0-a2a-poc` | 2025.07–2025.12 | 服务台与诊断角色交换任务、状态、结果和失败 | A2A 前期预研与本地 PoC，不描述为生产互操作 |
| `v1.0.0-multi-agent` | 2026.01–2026.09 | 形成分流、知识、诊断、工单和审计协同的完整服务台 | 多 Agent 工程化收敛；不等于分布式生产平台 |

每个标签都是可检出的真实代码状态，不把后续能力倒灌到前一版本。

项目经历的团队规模、个人职责、阶段分工、疑难问题、工程取舍和时间线门禁见 [项目经历口径](docs/项目经历口径.md)。当前 v0.1 的完整业务叙事见 [v0.1 业务与团队](docs/v0.1-业务与团队.md)。
